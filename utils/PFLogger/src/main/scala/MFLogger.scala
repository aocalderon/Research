...
object MFLogger {
  ...
  case class MFLog(timestamp: String, tag: String, appId: String, cores: Int,
    executors: Int, status: String, timer: Double, phase: String, time: Double,
    load: Int, instant: Int)
  ...
  def main(args: Array[String]): Unit = {
    ...
    implicit val params = new MFLoggerConf(args)
    ...
    val FF = timer{"Extracting FF data"}{
      val FF = lines.filter($"line".rlike("\\|MF\\|"))
        .filter($"line".rlike("\\|  END\\|")).cache

      val schema = ScalaReflection.schemaFor[MFLog].dataType.asInstanceOf[StructType]
      splitDF(FF, $"line", schema.fieldNames).cache()
    }

    val appParameters = Seq("mfpartitions", "dpartitions")
    ...
    val data = timer{"Joining"}{
      val columnsA = List($"A.appId", $"phase", $"time", $"instant")
...
class MFLoggerConf(args: Seq[String]) extends ScallopConf(args) {
