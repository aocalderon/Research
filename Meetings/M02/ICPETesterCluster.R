...
data_path = "~/Documents/PhD/Research/Meetings/M02/ICPETesterCluster.txt"
icpe = enframe(read_lines(data_path), name = "n", value = "line") %>% select(line) %>% 
  filter(grepl("app-2019", line)) %>%
...
ggsave("ICPETesterClusterGRIndex.pdf", width = 10, height = 6, device = "pdf")
...
ggsave("ICPETesterClusterDBScan.pdf", width = 10, height = 6, device = "pdf")