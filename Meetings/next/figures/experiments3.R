...
M=4
E=15
dataByEpsilonAndMu = data2 %>% filter(Mu == M && Epsilon==E)
p = ggplot(data = dataByEpsilonAndMu, aes(x = factor(Dataset), y = Duration, fill = Stage)) +
  geom_bar(stat="identity", position=position_dodge(width = 0.75), width = 0.7) + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
  labs(x="Dataset", y="Time [sec]", title=paste0("Performance by dataset (mu=", M,", epsilon=", E,")"))
 plot(p)
 ggsave(paste0("E_", E,"_M", M, ".pdf"), device = "pdf")
