...
fields2 <- c("ts", "epoch", "host", "tag", "n1", "appId", "n2", "dataset", "epsilon", "mu", "delta", "method", "time")

bfe <- enframe(read_lines("e/bfe_la25k_e.txt"), value = "line") |>
  filter(str_detect(line, 'TIME')) |>
...
psi <- enframe(read_lines("e/psi_la25k_e.txt"), value = "line") |>
  filter(str_detect(line, 'TIME')) |>
  separate(col = line, into = fields2, sep = "\\|") |>
  mutate(time = as.numeric(time), epsilon = as.numeric(epsilon)) |>
  select(epsilon, time) |>
