...
data_path = "~/Documents/PhD/Research/Meetings/M01/ICPE_B43K-44K.txt"
...
icpe1 = enframe(read_lines(data_path), name = "n", value = "line") %>% select(line) %>% 
  filter(grepl("ICPE", line)) %>% mutate(n = rep(1:1005, each = 16)) 
...
p = ggplot(data = data, aes(x = factor(Epsilon), y = Time, group = Method)) +
  ...
  labs(title="Brinkhoff sample 2 dataset", x="Epsilon [m]", y="Time [s]") 
plot(p)

ggsave("ICPE_B43K-44K.pdf", width = 10, height = 6, device = "pdf")