...
log = enframe(readLines("ByIndexer4.txt"))
...
data1 = data %>% filter(Fraction == "0.05") %>% group_by(Phase, Capacity, Fraction) %>% summarise(Time = mean(Time))
...
data2 = data %>% filter(Capacity == "3000") %>% group_by(Phase, Fraction) %>% summarise(Time = mean(Time))
