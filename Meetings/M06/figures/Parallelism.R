...
spark = spark %>% separate(params, into = c(NA,"Epsilon", NA,"Partitions", NA,"Parallelism", NA,"Partitions2"), sep = " ") %>%
  select(appId, Epsilon, Partitions, Parallelism) 
...
data1 = data %>% group_by(Phase, Partitions, Parallelism, Epsilon) %>% summarise(Time = mean(Time))
data2 = data1 %>% group_by(Partitions, Parallelism, Epsilon) %>% summarise(Time = sum(Time))

p = ggplot(data = data2, aes(x = factor(Partitions), y = Time, fill = Epsilon)) +
  geom_bar(stat="identity", position=position_dodge(width = 0.75), width = 0.7) +
  facet_wrap(~Parallelism, ncol = 1) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
  labs(x="Number of partitions", y="Time [s]", title="Total execution time MF by parallelism") 
plot(p)
ggsave("Parallelism.pdf", width = 8, height = 7, device = "pdf")  