...
object MoSTTrajExtractor extends Logging {
  ...
  def runSparkJob(S: Settings): Unit = {
    ...
    try {
      ...
      val A: Dataset[List[STPoint]] = points
        .mapPartitions { iter =>
          iter.toList
            .groupBy(_.oid)
            .flatMap { case (oid, list) =>
              val sorted                        = list.sortBy(_.tid)
              val splitted: List[List[STPoint]] =
                Utils.splitSTPointByGap(sorted).filter(_.length >= minimum_duration).map { segment =>
