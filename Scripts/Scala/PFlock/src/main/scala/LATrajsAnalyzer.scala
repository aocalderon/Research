...
object LATrajsAnalyzer {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val dataset = params.dataset().split("/").last.split("\\.").head

    logger.info("Reading data...")
    val pointsRDD = spark.read
      .textFile(params.dataset())
      .rdd.map { line =>
        val arr = line.split("\t")
        val i = arr(0).toInt
    ...
    if(params.debug()){
      ...
      val f = new java.io.FileWriter(s"/tmp/edgesCH_${dataset}.wkt")
    ...
    val duplicates = pointsRDD.mapPartitions{ it =>
      ...
      points.map{ point =>
        ...
        val hood = tree.query(envelope).asScala.map(_.asInstanceOf[STPoint2]).toList
        hood.filter(_.oid != point.oid).map{ p2 => (point, p2, point.distance(p2)) }
      }.flatten.toIterator
    }.cache

    {
      val f = new java.io.FileWriter(s"/tmp/edgesDu_${dataset}.wkt")
