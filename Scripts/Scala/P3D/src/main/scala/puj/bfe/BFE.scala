...
object BFE extends Logging {
  ...
  def run(points_prime: List[STPoint], maximals_found: RTree[Disk] = RTree.empty)(implicit S: Settings, G: GeometryFactory): (List[Disk], Stats) = {
    ...
    if (points_prime.isEmpty) {
      ...
      grid.index.keys.foreach { key =>
        val ((_Pr, _Ps), tRead) = timer {
          ...
          val Ps = indices.flatMap {
            case (i, j) => // getting points around current cell...
              val key = encode(i, j)
