...
object PF_Utils extends Logging{
  ...
  def processUpdates(flocksRDD: RDD[Disk], cells: Map[Int, Cell], n: Int = 1, i: Int = 1)
    (implicit S: Settings, G: GeometryFactory, P: Params):
  ...
  def process(flocks: RDD[Disk], cells: Map[Int, Cell], n: Int = 1, i: Int = 1)
    (implicit S: Settings, G: GeometryFactory, P: Params): (RDD[Disk], Map[Int, Cell]) = {
    if(cells.size > 1){
  ...
  private def updateCellsSec(cells: Map[Int, Cell])(implicit G: GeometryFactory): Map[Int, Cell] = {
    ...
    }.zipWithIndex.map{ case(cell, id) => id -> cell.copy(id = id) }.toMap
  }
