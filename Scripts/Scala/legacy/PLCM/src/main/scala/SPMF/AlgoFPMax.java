...
public class AlgoFPMax {
    ...
    private Comparator<Integer> comparatorOriginalOrder = (item1, item2) -> {
	...
	public Itemsets runAlgorithm(String input, String output, double minsupp) throws IOException {
		...
		while( ((line = reader.readLine())!= null)){ 
			...
			transaction.sort(comparatorOriginalOrder);
...
*/
	...
	private void fpMax(FPTree tree, int [] prefix, int prefixLength, int prefixSupport, Map<Integer, Integer> mapSupport) throws IOException {
		...
		if(singlePath && singlePathSupport >= minSupportRelative){	
			...
			for(int i = tree.headerList.size()-1; i>=0; i--){
				...
				// to get head U P
				List<Integer> headWithP = new ArrayList<>(mapSupportBeta.size() + prefixLength + 1);
				...
				// sort item in the transaction by descending order of support
				headWithP.sort(comparatorOriginalOrder);
				...
				if(mfiTree.passSubsetChecking(headWithP)) {
					...
					temp.sort(comparatorOriginalOrder);
	...
	private void saveItemset(int [] itemset, int itemsetLength, int support) throws IOException {
		...
		mfiTree.addMFI(itemsetCopy, itemsetCopy.length);
