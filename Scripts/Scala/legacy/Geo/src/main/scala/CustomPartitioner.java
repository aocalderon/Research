...
public class CustomPartitioner extends SpatialPartitioner
{
    public CustomPartitioner(GridType gridType, List<Envelope> grids)
    {
        super(gridType, grids);
    }

    // For backwards compatibility (see SpatialRDD.spatialPartitioning(otherGrids))
    public CustomPartitioner(List<Envelope> grids)
    {
        super(null, grids);
    }
    ...
    {
        if (o == null || !(o instanceof CustomPartitioner)) {
            return false;
        }

        final CustomPartitioner other = (CustomPartitioner) o;
