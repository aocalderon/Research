...
object PF_Utils extends Logging {
          ...
          count match {
            case count if flock1.pids.size == count =>
              if (flock2.start <= flock1.start && flock1.end <= flock2.end) {
                ...
                if (flock2.start < flock1.start || flock1.end < flock2.end) {
                  // flock2 is not a subset of flock1 (they have same pids but differ in time). We
                  // iterate next..
                } else {
  ...
  def parPrune(flocks: List[Disk])(implicit
    spark: SparkSession,
    S: Settings,
    cubes: Map[Int, Cube],
    tree: StandardQuadTree[Point]
  ): List[Disk] = {
    val n     = tree.getLeafZones.size()
    val cells = tree.getLeafZones().asScala.map {
      leaf =>
    ...
    spark.sparkContext
      ...
      .flatMap {
        flock =>
          ...
          tree.findZones(new QuadRectangle(envelope)).asScala.map {
            zone =>
      ...
      .mapPartitionsWithIndex {
        (index, it) =>
          ...
          val r         = pruneByArchery(flocks).filter {
            f =>
              cell.contains(f)
          }.toIterator
  ...
  def pruneByArchery(flocks: List[Disk])(implicit S: Settings): List[Disk] = {
    ...
    val f    = flocks.sortBy(_.start).zipWithIndex.map {
      case (flock, id) =>
        flock.id = id
  ...
  @tailrec
  private def pruneByArcheryRec(
    Q: List[Disk],
    Q_prime: List[Disk],
    tree_prime: RTree[Disk],
    S: Settings
  ): List[Disk] = {
  ...
  @tailrec
  private def pruneByArcheryRec2(
    Q: List[Disk],
    Q_prime: List[Disk],
    tree_prime: RTree[Disk],
    S: Settings
    ...
    tree_prime.entries.toList match {
      case flock_entry :: tail =>
        ...
        for { flock2_entry <- zone if !stop } {
          ...
          tree =
            if (flock1.start == flock2.start && flock1.end == flock2.end && flock1.pidsText == flock2.pidsText) {
              stop = true
              if (flock1 < flock2) {
                tree
              } else {
              ...
              count match {
                case count if flock1.pids.size == count =>
                  if (flock2.start <= flock1.start && flock1.end <= flock2.end) {
                    ...
                    if (flock2.start < flock1.start || flock1.end < flock2.end) {
                      // flock2 is not a subset of flock1 (they have same pids but differ in
                      // time). We iterate next..
                      tree
                    } else {
                      flock2.subset =
                        true // flock2 is a subset of flock1.  We need to remove it...
                      tree.remove(flock2_entry)
                    }
                  }
                case count if flock2.pids.size == count =>
  ...
  @tailrec
  def join(trajs: List[(Int, Iterable[STPoint])], flocks: List[Disk], f: List[Disk])(implicit
    S: Settings,
    P: Params,
    G: GeometryFactory
  ): List[Disk] = {
    ...
    trajs match {
      case current_trajs :: remaining_trajs =>
        ...
        val N = new_flocks.filterNot(_.subset).map {
          flock =>
    ...
    trajs match {
      case current_trajs :: remaining_trajs =>
        ...
        val (new_flocks, stats) = if (S.method == "BFE") {
          ...
          (
            nf.map(_.copy(start = time, end = time)).filter(d => cell_prime.contains(d.center.getCoordinate)),
            stats
          )
        ...
        val F_prime = candidates
          .map {
            flock =>
              ...
              val safe_area  = cell.contains(flock.locations.head) || cell.contains(
                flock.locations.last
              ) // report if flock in in safe area
  ...
  def joinDisksCachingPartials(
    ...
    trajs match {
      case current_trajs :: remaining_trajs =>
        ...
        val new_flocks                = new_flocks_prime.map(_.copy(start = time, end = time)).filter(
          d => cell_prime.contains(d.center.getCoordinate)
        )
        ...
        val F_prime = candidates
          .map {
            flock =>
              ...
              val safe_area  = cell.contains(flock.locations.head) || cell.contains(
                flock.locations.last
              ) // report if flock is in safe area...
        ...
        val T_partial_end   = if (time > time_end - (S.delta - 1)) {
          val prime = candidates.filter { candidate => candidate.end == time }.map {
            f =>
              if (f.start < time) {
        ...
        val F_prime = candidates
          .map {
            flock =>
              val safe_delta = flock.end - flock.start >= S.delta - 1
              val safe_area  =
                flock.locations.exists(c => cell.contains(c)) // report if a flock location is in safe area
  ...
  def funPartial(f: List[Disk], time: Int, partials: mutable.HashMap[Int, (List[Disk], STRtree)], result: List[Disk])(
    ...
    val t4      = clocktime
    val F_prime = candidates.map {
      f =>
        val a = f.start == time - (S.delta - 1)
        (f, a)
    }
    val r       = F_prime.filter(_._2).map {
      case (f, _) =>
  ...
  def funPartial2(
    f: List[Disk],
    time: Int,
    partials: mutable.HashMap[Int, (List[Disk], STRtree)],
    result: List[Disk]
  ...
  def processUpdates(flocksRDD: RDD[Disk], cells: Map[Int, Cell], n: Int = 1, i: Int = 1)(implicit
    S: Settings,
    G: GeometryFactory,
    P: Params
    ...
    // t0 = clocktime
    val B = ARDD.mapPartitionsWithIndex {
      (index, F_prime) =>
        ...
        /* val f1 = flocks.toList save(s"/tmp/edgesF_C${i}_I${index}.wkt"){ f1.map{ f => s"${f.wkt}\n" } } val p1 =
         * p_prime.toList save(s"/tmp/edgesP_C${i}_I${index}.wkt"){ p1.map{ f => s"${f.wkt}\n" } } val s1 =
         * still_partials.toList save(s"/tmp/edgesS_C${i}_I${index}.wkt"){ s1.map{ f => s"${f.wkt}\n" } } */
  ...
  @tailrec
  def process(flocks: RDD[Disk], cells: Map[Int, Cell], n: Int = 1, i: Int = 1)(implicit
    S: Settings,
    G: GeometryFactory,
    P: Params
  ): (RDD[Disk], Map[Int, Cell]) = {
  ...
  def updateCells(cells: Map[Int, Cell], e: Int)(implicit G: GeometryFactory): Map[Int, Cell] =
    updateCells2(cells, 0, e)
  ...
  private def updateCellsSec(cells: Map[Int, Cell])(implicit G: GeometryFactory): Map[Int, Cell] = {
    cells.values
      ...
      .flatMap {
        case (lid, cells) =>
          if (cells.size == 4) {
            val env = cells.map(c => new Envelope(c.envelope)).reduce {
              (a, b) =>
                a.expandToInclude(b)
                a
            }
