...
object MF_Utils extends Logging {
  ...
  def decode(z: Long): (Int, Int) = {
    ...
    return if (a < b) { (a.toInt, b.toInt) }
    else { (b.toInt, (a - b).toInt) }
  }
  ...
  def insertMaximal(maximals: archery.RTree[Disk], candidate: Disk)(implicit S: Settings): archery.RTree[Disk] = {
  ...
  def getPointsAroundCenter(center: Point, points: List[STPoint])(implicit S: Settings, G: GeometryFactory): Disk = {
    val pids = for {
