...
speed = args.speed
## Running PFlock...
command = "spark-submit --class FlockFinderBenchmark {0} --master local[*] --split 0.9 --epsilon {1} --mu {2} --delta {3} --path {4} --dataset {5} --speed {6} --debug".format(pflock, epsilon, mu, delta, path, dataset, speed)
if(args.pflock):
  subprocess.call(command, shell=True)
