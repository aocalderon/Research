...
command = "~/Research/Scripts/Python/DiskVisualizer.py -e {0} -m {1} -d {2} -p {3}".format(epsilon, mu, delta, pointset)
subprocess.call(command, shell=True)
command = "~/Research/Scripts/Python/NotFoundFlockSaver.py -e {0} -m {1} -d {2} -p {3}".format(epsilon, mu, delta, pointset)
subprocess.call(command, shell=True)

command = "spark-submit {0} {1} {2}".format(flock_checker, bfe_output, pflock_output)
subprocess.call(command, shell=True)
notfound = open("/tmp/NotFound.flocks", "r") 
line = notfound.readline()
if line !=  "":
  command = "~/Research/Scripts/Python/DiskVisualizer.py -e {0} -m {1} -d {2}".format(epsilon, mu, delta)
  subprocess.call(command, shell=True)
  print("ERROR: Flocks not found comparing BFE vs PFlock.  See /tmp/NotFound.flocks for details...")
