...
## Running PFlock...
command = "spark-submit --class FlockFinderBenchmark {0} --epsilon {1} --epsilon_max {1} --mu {2} --mu_max {2} --delta {3} --delta_max {3} --path {4} --dataset {5} --debug --master local[*]".format(pflock, epsilon, mu, delta, path, dataset)
print(command)
if(args.pflock):
  subprocess.call(command, shell=True)
