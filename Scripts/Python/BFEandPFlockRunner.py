...
parser.add_argument("--max_epsilon", "-e2", default=30.0, help="Maximum epsilon...")
...
parser.add_argument("--step_mu", "-sm", default=2, help="Step in mu...")
...
for delta in range(min_delta, max_delta + 1, step_delta):
  for epsilon in range(min_epsilon, max_epsilon + 1, step_epsilon):
    for mu in range(min_mu, max_mu + 1, step_mu):
      ...
      command = "{0} -p {1} -i {2} -e {3} -m {4} -d {5} --no_bfe".format(runner_script
      , args.path
      , args.dataset
      , epsilon
      , mu
