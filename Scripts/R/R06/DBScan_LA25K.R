...
data_path = "~/Documents/PhD/Research/Scripts/R/R06/DBScan_LA25K.txt"
...
p = ggplot(data = data, aes(x = factor(Epsilon), y = Time, group = Method)) +
  ...
  labs(title="LA_25K dataset", x="Epsilon [m]", y="Time [s]") 
plot(p)

ggsave("LA25K.pdf", width = 7, height = 4, device = "pdf")