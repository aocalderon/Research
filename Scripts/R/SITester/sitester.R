...
data = enframe(read_lines("~/Research/Scripts/Scala/SITester/sitester3.txt"), value = "line") |>
...
p = ggplot(data1, aes(x = metric, y = time, fill = type)) +
...
p = ggplot(data1, aes(x = as.factor(metric), y = time, group = type)) +
  geom_line(aes(linetype = type, color = type)) + 
