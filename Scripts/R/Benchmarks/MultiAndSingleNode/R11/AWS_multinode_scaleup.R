...
data0$Nodes <- factor(data0$Nodes, levels = c("1", "2", "3", "4"))

data1 = data0 %>% group_by(Nodes, Epsilon) %>% summarise(Time = mean(Time2), SD = sd(Time2))

title = "Multinode Scale Up by Epsilon [Berlin_N40K, Berlin_N80K, Berlin_N120K, Berlin_N160K, 4 cores per node, 1 thread per core]"
...
title = "Multinode Scale Up by Epsilon Boxplot [Berlin_N40K, Berlin_N80K, Berlin_N120K, Berlin_N160K, 4 cores per node, 1 thread per core]"
...
title = "Multinode Scale Up by Epsilon without outliers [Berlin_N40K, Berlin_N80K, Berlin_N120K, Berlin_N160K, 4 cores per node, 1 thread per core]"
