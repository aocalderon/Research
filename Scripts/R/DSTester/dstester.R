...
W = 6
H = 4
ggsave(paste0("DBScanByTime.pdf"), width = W, height = H)

p = ggplot(data, aes(x = as.factor(epsilon), y = clusters, group = implementation)) +
  geom_line(aes(color = implementation, linetype = implementation)) + 
  geom_point(aes(color = implementation, shape = implementation), size = 3) +
  labs(x="Epsilon(m)", y="Number of clusters") +
  scale_color_discrete("Implementation") +
  scale_shape_discrete("Implementation") +
  guides(linetype = "none") +
  theme_bw()
plot(p)  
ggsave(paste0("DBScanByNumberOfClusters.pdf"), width = W, height = H)