...
data_path = "~/Documents/PhD/Research/Scripts/R/R05/nohup001.txt"
...
plot(p)
ggsave("LA_10K_MaximalsLatency.pdf", device = "pdf")
...
plot(p)
ggsave("LA_10K_MaximalsThroughput.pdf", device = "pdf")
...
p = ggplot(data = joins, aes(x = factor(Epsilon), y = Time, group = 1)) +
  ...
  labs(x="Epsilon [m]", y="Average latency [s]") 
plot(p)
ggsave("LA_10K_JoinLatency.pdf", device = "pdf")
...
p = ggplot(data = maximals2, aes(x = factor(Epsilon), y = tps, group = 1)) +
  ...
  labs(x="Epsilon [m]", y="Throughput [tps]") 
plot(p)
ggsave("LA_10K_JoinThroughput.pdf", device = "pdf")
