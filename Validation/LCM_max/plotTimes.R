...
data = read.table('/home/and/Documents/PhD/Research/Validation/LCM_max/times.txt')
n = nrow(data)
method = rep(c('FPMax', 'LCM'), n/2)
dataset = rep(seq(1,143), each=2)
...
g = ggplot(data=counts, aes(x=Dataset, y=Time, color=Method)) +
  geom_line(alpha=0.8) +
  geom_hline(yintercept = mean(counts[counts$Method=='FPMax', "Time"]), linetype="dashed", color='blue') +
