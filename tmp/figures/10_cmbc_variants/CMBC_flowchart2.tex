...
\begin{document}
    \begin{tikzpicture}[node distance = 2cm, auto]
        % Place nodes
        \node [block] (graph) {Find pairs};
        \node [cloud, left of=graph, node distance=3cm] (input) {$\mathcal{P}oints$, $\varepsilon$, $\mu$};
        ...
        \node [block, below left of=decide, node distance=3cm, text width=6em] (traditional) {Prune cliques};
        ...
        \node [cloud, below of=traditional, node distance=2cm] (maximals) {$\mathcal{M}aximals\_disks$};
        % Draw edges
