...
case class Flock(items: Vector[Int], start: Int, end: Int, center: Point){
  def getItemset: Set[Int] = items.toSet

  def getItems: Vector[Int] = items.sorted
  ...
  def toTSV(): String = f"$start\t$end\t${center.getX}%.3f\t${center.getY}%.3f\t${getItems.mkString(" ")}"

  def toCSV(): String = s"$start, $end, ${getItems.mkString(" ")}"

  def toWKT: String = s"POINT(${center.getX}, ${center.getY})\t${this.toString()}"

  // Override equals to allows difference in centers...
...
case class TDisk(t: Int, disk: Disk) {
  ...
  def getItems: Vector[Int] = disk.getItems
...
case class Disk(x: Double, y: Double, pids: Set[Int], var subset: Boolean = false) {
  ...
  def getItems: Vector[Int] = pids.toVector.sorted
